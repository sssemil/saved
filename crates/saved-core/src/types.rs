...
pub enum Event {
    ...
    PeerLost { device_id: String, reason: String },
    ...
    /// Peer tag added
    PeerTagAdded { device_id: String, tag: String },
    /// Peer tag removed
    PeerTagRemoved { device_id: String, tag: String },
    /// Network listening started
    NetworkListeningStarted { addresses: Vec<String> },
    ...
    RelayConnectionAttempt { relay_address: String },
...
impl AccountHandle {
    ...
    pub async fn create_or_open_with_account_key(
        config: Config,
        account_key: Option<crate::crypto::AccountKey>,
    ) -> crate::Result<Self> {
        ...
        let mut sync_manager =
            sync::SyncManager::new(storage, config.storage_path.clone(), vault_key, device_key);
        ...
        if let Some(account_key) = account_key {
            ...
            sync_manager
                .storage_mut()
                .store_account_key(encrypted_account_key.as_bytes())
                .await?;
            ...
            sync_manager
                .storage_mut()
                .store_account_key_info(&key_info)
                .await?;
        }
    ...
    pub async fn authorize_device(
        ...
        if !self.has_account_private_key().await? {
            return Err(crate::error::Error::Crypto(
                "This device does not have the account private key to authorize new devices"
                    .to_string(),
            ));
        }
        ...
        if let Some(key_info) = self.get_account_key_info().await? {
            ...
            self.sync_manager
                .storage_mut()
                .store_authorized_device(&device_id, &cert_bytes)
                .await?;
            ...
            self.event_sender.send(event).map_err(|e| {
                crate::error::Error::Network(format!("Failed to send authorization event: {}", e))
            })?;

            Ok(())
        } else {
            Err(crate::error::Error::Crypto(
                "No account key info found".to_string(),
            ))
        }
    }
    ...
    pub async fn revoke_device(&mut self, device_id: &str) -> crate::Result<()> {
        ...
        self.sync_manager
            .storage_mut()
            .revoke_device_authorization(device_id)
            .await?;

        // Send device revocation event
        let event = Event::DeviceRevoked(device_id.to_string());
        self.event_sender.send(event).map_err(|e| {
            crate::error::Error::Network(format!("Failed to send revocation event: {}", e))
        })?;
        ...
        if let Some(ref mut network_manager) = self.network_manager {
            ...
            network_manager
                .force_disconnect_peer(device_id.to_string())
                .await?;
        }
    ...
    pub async fn is_device_authorized(&self, device_id: &str) -> crate::Result<bool> {
        self.sync_manager
            .storage()
            .is_device_authorized(device_id)
            .await
    }
    ...
    pub async fn share_account_key(&mut self, _target_device_id: &str) -> crate::Result<Vec<u8>> {
        ...
        if !self.has_account_private_key().await? {
            return Err(crate::error::Error::Crypto(
                "This device does not have the account private key".to_string(),
            ));
        }

        // Get the encrypted account key from storage
        let encrypted_account_key = self
            .sync_manager
            .storage()
            .get_account_key()
            .await?
            .ok_or_else(|| {
                crate::error::Error::Crypto("Account key not found in storage".to_string())
            })?;

        // Store the shared account key for the target device
        self.sync_manager
            .storage_mut()
            .store_shared_account_key(&encrypted_account_key)
            .await?;

        // Encrypt the account key with the target device's public key
        let device_encrypted_key = self
            .encrypt_account_key_for_device(&encrypted_account_key, _target_device_id)
            .await?;
    ...
    pub async fn accept_shared_account_key(
        &mut self,
        ...
        self.sync_manager
            .storage_mut()
            .store_account_key(encrypted_account_key)
            .await?;
        ...
        if let Some(mut key_info) = self.get_account_key_info().await? {
            ...
            let public_key = self
                .extract_public_key_from_encrypted_key(encrypted_account_key)
                .await?;
    ...
    async fn encrypt_account_key_for_device(
        &self,
        account_key: &[u8],
        ...
        for (device_id, cert_bytes) in authorized_devices {
            if device_id == target_device_id {
                ...
                let device_cert: crate::crypto::DeviceCert = bincode::deserialize(&cert_bytes)
                    .map_err(|e| {
                        crate::error::Error::Crypto(format!(
                            "Failed to deserialize device cert: {}",
                            e
                        ))
                    })?;
                ...
                let encrypted_key =
                    self.encrypt_for_device(account_key, &device_cert.device_pubkey)?;
        ...
        Err(crate::error::Error::Crypto(format!(
            "Target device {} not found in authorized devices",
            target_device_id
        )))
    }

    /// Extract public key from encrypted account key
    async fn extract_public_key_from_encrypted_key(
        &self,
        encrypted_key: &[u8],
    ) -> crate::Result<[u8; 32]> {
        ...
        if encrypted_key.len() < 64 {
            // ephemeral_public_key (32) + nonce (24) + minimum encrypted data
            return Err(crate::error::Error::Crypto(
        ...
        if decrypted_key.len() < 32 {
            return Err(crate::error::Error::Crypto(
                "Decrypted account key too short".to_string(),
            ));
        }
    ...
    pub async fn get_account_key_info(
        &self,
    ) -> crate::Result<Option<crate::crypto::AccountKeyInfo>> {
    ...
    pub async fn authorize_device_with_account_key(
        &mut self,
        device_cert: crate::crypto::DeviceCert,
    ) -> crate::Result<()> {
        // Check if this device has the account private key
        ...
        if let Some(key_info) = self.get_account_key_info().await? {
            ...
            return Err(crate::error::Error::Crypto(
                "No account key info found for certificate verification".to_string(),
            ));
        }
    ...
    pub async fn make_linking_qr(&self) -> crate::Result<QrPayload> {
        ...
        let device_id = format!(
            "device-{}",
            hex::encode(&device_key.public_key_bytes()[..8])
        );
    ...
    pub async fn accept_link(&self, payload: QrPayload) -> crate::Result<DeviceInfo> {
        ...
        let is_authorized = if let Some(device_cert) = &payload.device_cert {
            ...
            if let Some(key_info) = self.get_account_key_info().await? {
                let account_key =
                    crate::crypto::AccountKey::from_public_bytes(&key_info.public_key)?;
        ...
        if is_authorized {
            if let Some(device_cert) = &payload.device_cert {
                ...
                let cert_bytes = bincode::serialize(device_cert).map_err(|e| {
                    crate::error::Error::Storage(format!("Failed to serialize device cert: {}", e))
                })?;

                // Store the authorized device
                self.sync_manager
                    .storage()
                    .store_authorized_device(&payload.device_id, &cert_bytes)
                    .await?;
            }
        }
    ...
    async fn get_decrypted_account_key(&self) -> crate::Result<crate::crypto::AccountKey> {
        ...
        Err(crate::error::Error::Storage(format!(
            "Invalid account key length: {}",
            account_key_bytes.len()
        )))
    }
    ...
    pub async fn start_network(&mut self) -> crate::Result<()> {
        ...
        for (device_id, cert_bytes) in authorized_devices {
            let device_cert: crate::crypto::DeviceCert = bincode::deserialize(&cert_bytes)
                .map_err(|e| {
                    crate::error::Error::Crypto(format!(
                        "Failed to deserialize device cert for {}: {}",
                        device_id, e
                    ))
                })?;
        ...
        if self.network_manager.is_none() {
            ...
            let network_manager =
                crate::networking::NetworkManager::new(device_key, network_event_sender).await?;
    ...
    pub async fn force_announce(&self) {
        ...
        if let Some(ref network_manager) = self.network_manager {
            ...
            let healthy_peers = network_manager
                .get_peers_by_health(&crate::networking::PeerHealth::Healthy)
                .await;
            let unhealthy_peers = network_manager
                .get_peers_by_health(&crate::networking::PeerHealth::Unhealthy)
                .await;
    ...
    pub async fn discovered_peers(
        &self,
    ) -> std::collections::HashMap<String, crate::networking::DiscoveryInfo> {
        if let Some(ref nm) = self.network_manager {
    ...
    pub async fn peer_connection_state(&self, device_id: &str) -> Option<String> {
        if let Some(ref nm) = self.network_manager {
            nm.get_peer_connection_state(device_id)
                .await
                .map(|s| format!("{:?}", s))
        } else {
            None
    ...
    pub async fn add_discovered_peer(
        ...
        device_id: String,
        addresses: Vec<String>,
        method: crate::networking::DiscoveryMethod,
    ) -> crate::Result<()> {
    ...
    pub async fn scan_local_network(
        &mut self,
    ) -> crate::Result<Vec<crate::networking::DiscoveryInfo>> {
        if let Some(ref mut nm) = self.network_manager {
    ...
    pub async fn store_chunk(&self, data: &[u8]) -> crate::Result<crate::storage::sqlite::ChunkId> {
        if let Some(chunk_sync_manager) = &self.chunk_sync_manager {
            ...
            Err(crate::error::Error::Storage(
                "Chunk sync not initialized".to_string(),
            ))
        }
    }
    ...
    pub async fn get_chunk(
        &self,
        chunk_id: &crate::storage::sqlite::ChunkId,
    ...
    pub async fn check_chunk_availability(
        &self,
        chunk_ids: &[crate::storage::sqlite::ChunkId],
    ) -> crate::Result<std::collections::HashMap<crate::storage::sqlite::ChunkId, bool>> {
        if let Some(chunk_sync_manager) = &self.chunk_sync_manager {
...
mod tests {
    ...
    async fn test_device_linking_flow() {
        ...
        let mut account = AccountHandle::create_account_key_holder(config)
            .await
            .unwrap();
