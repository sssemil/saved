...
pub async fn link_command(
    ...
    if let Some(output_path) = output {
        ...
        println!("QR payload saved to: {}", output_path.display().to_string().bright_blue());
...
pub async fn devices_command(account_path: &PathBuf, verbose: bool) -> Result<()> {
    ...
    println!("\n{}", "Note:".yellow());
    println!("Device discovery and connection management is not yet implemented in the core library.");
