        ...
        match swarm.next().await.expect("Infinite Stream.") {
            SwarmEvent::Behaviour(event) => {
                ...
                match &event {
                    ...
                    BehaviourEvent::Relay(relay::Event::CircuitReqAccepted { src_peer_id, dst_peer_id, .. }) => {
                        info!("🔗 Circuit established: {} -> {}", src_peer_id, dst_peer_id);
                    }
                    BehaviourEvent::Relay(relay::Event::CircuitReqDenied { src_peer_id, .. }) => {
                        warn!("🚫 Circuit request denied from peer: {}", src_peer_id);
                    }
                    BehaviourEvent::Relay(relay::Event::CircuitClosed { src_peer_id, dst_peer_id, .. }) => {
